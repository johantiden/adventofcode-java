...
""";
    ...
    private static int getBingoScore(Pair<JList<Bingo.Board>, JList<Integer>> winningState) {
        assert winningState.left().size() == 1;
        ...
        Integer last = winningSequence.last();
        System.out.println("last=" + last);

        return sum * last;
    }

    private static Pair<JList<Bingo.Board>, JList<Integer>> findWinners(Bingo bingo) {
        for (int i = 0; i < bingo.generator.size(); i++) {
            Bingo bingoLimited = new Bingo(bingo.generator.head(i), bingo.boards);
            JList<Bingo.Board> winners = findWinnersImmediate(bingoLimited);
            if (!winners.isEmpty()) {
                return new Pair<>(winners, bingoLimited.generator);
            }
        }
        throw new IllegalStateException();
    }

    private static JList<Bingo.Board> findWinnersImmediate(Bingo bingoLimited) {
        return bingoLimited.boards()
                .filter(board -> isWinner(board, bingoLimited.generator));
    }
    ...
    private static Bingo parse(String input) {
        ...
        JList<Bingo.Board> boards = split.tail()
                .map(A2021_04::parseBoard);

        return new Bingo(generator, boards);
    }
